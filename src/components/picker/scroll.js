...
$.fn.scroll = function (options) {
    const defaults = $.extend({
        items: [],                                  // 数据
        scrollable: '.don-picker__content',        // 滚动的元素
    ...
    const items = defaults.items.map((item) => {
        return `<div class="don-picker__item${item.disabled ? ' don-picker__item_disabled' : ''}">${item.label}</div>`;
    }).join('');
    const $this = $(this);

    $this.find('.don-picker__content').html(items);
