...
class Picker extends Component {
    ...
    renderActions(){
        let elActions = this.state.actions.map( (action, i)=> {
            ...
            return <a {...others} key={i} className="mgjc-picker__action"> { label }</a>;
        });

        return (
            <div className="mgjc-picker__hd">
                { elActions }
            </div>
        );
    }
    ...
    render(){
        ...
        const cls = classNames('mgjc-picker', {
            'mgjc-animate-slide-up': show && !this.state.closing,
            'mgjc-animate-slide-down': this.state.closing
        }, className);

        const maskCls = classNames({
            'mgjc-animate-fade-in': show && !this.state.closing,
            'mgjc-animate-fade-out': this.state.closing
        });
