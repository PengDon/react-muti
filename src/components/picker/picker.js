...
function picker() {
    ...
    function show(){
        ...
        $picker.find('.don-mask').addClass('don-animate-fade-in');
        $picker.find('.don-picker').addClass('don-animate-slide-up');
    }
    function _hide(callback){
        _hide = $.noop; // 防止二次调用导致报错

        $picker.find('.don-mask').addClass('don-animate-fade-out');
        $picker.find('.don-picker')
            .addClass('don-animate-slide-down')
    ...
    function scroll(items, level) {
        ...
        $picker.find('.don-picker__group').eq(level).scroll({
            items: items,
            temp: lineTemp[level],
            onChange: function (item, index) {
                ...
                if (isMulti) {
                    ...
                    if (item.children && item.children.length > 0) {
                        $picker.find('.don-picker__group').eq(level + 1).show();
                        !isMulti && scroll(item.children, level + 1); // 不是多列的情况下才继续处理children
                    } else {
                        //如果子列表test不通过，子孙列表都隐藏。
                        const $items = $picker.find('.don-picker__group');
    ...
    $picker.find('.don-picker__bd').html(groups);
    show();
    ...
    $picker
        .on('click', '.don-mask', function () { hide(); })
        .on('click', '.don-picker__action', function () { hide(); })
        .on('click', '#don-picker-confirm', function () {
