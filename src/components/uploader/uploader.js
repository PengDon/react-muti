 ...
 *            don.alert('请上传不超过10M的图片');
 *            return false;
 *        }
 *        if (files.length > 5) { // 防止一下子选择过多文件
 *            don.alert('最多只能上传5张图片，请重新选择');
 *            return false;
 *        }
 *        if (uploadCount + 1 > 5) {
 *            don.alert('最多只能上传5张图片');
 *            return false;
 *        }
 *
 *        ++uploadCount;
...
function uploader(selector, options) {
    ...
    function findFileCtn($uploader, id){
        ...
        let $fileCtn = $file.find('.don-uploader__file-content');

        if(!$fileCtn.length){
            $fileCtn = $('<div class="don-uploader__file-content"></div>');
            $file.append($fileCtn);
        }
        $file.addClass('don-uploader__file_status');
        return $fileCtn;
    }

    // 清除DOM里的上传状态
    function clearFileStatus($uploader, id){
        const $file = $uploader.find('[data-id="' + id + '"]').removeClass('don-uploader__file_status');
        $file.find('.don-uploader__file-content').remove();
    }
    ...
    if(options.onBeforeQueued){
        ...
        options.onBeforeQueued = function(file, files){
            ...
            $uploader.find('.don-uploader__files').append($item);
        };
    }
    if(options.onQueued){
    ...
    if(options.onError){
        ...
        options.onError = function(file, err){
            ...
            if(!onError.call(file, err)){
                findFileCtn($uploader, file.id).html('<i class="don-icon-warn"></i>');
            }
        };
    }
