...
function _findCellParent(ele){
    ...
    if(ele.classList.contains('don-cell')) return ele;
...
function _showErrorMsg(error){
    if(error){
        ...
        if(cellParent) cellParent.classList.add('don-cell_warn');
    }
}
 ...
 *
 * ``` js
 * // don.form.validate('#form', function(error){ console.log(error);}); // error: {dom:[Object], msg:[String]}
 * don.form.validate('#form', function (error) {
 *     if (!error) {
 *         var loading = don.loading('提交中...');
 *         setTimeout(function () {
 *             loading.hide();
 *             don.toast('提交成功', 3000);
 *         }, 1500);
    ...
    $eles.forEach((ele) => {
        ...
        $form.find('[required]')
            ...
            .on('focus', function () {
                ...
                if(cellParent) cellParent.classList.remove('don-cell_warn');
            });
    });
